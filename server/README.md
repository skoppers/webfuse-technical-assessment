...
| `internal/webhook/` | The security boundary for Space lifecycle webhooks: `ReadBody` (gunzip, size cap), `Verify` (hex-encoded HMAC-SHA256 over the decompressed body), `Parse` into `Envelope` plus `SessionData`/`ParticipantData`, the `RequireSignature` middleware, and the `POST /webhooks/webfuse` handler that routes each verified envelope by category to `Lifecycle.Started`/`Ended`/`ParticipantsChanged`. Owns the webhook wire shapes; stores and publishes nothing itself. |
